    ...
    private func startRetryTimer() {
        ...
        retryTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { [weak self] timer in
            Task { @MainActor in
                ...
                if (!self.activeRecords.isEmpty && !self.activityTypes.isEmpty) || self.retryCount >= self.maxRetries {
                    print("[Watch VM] Stopping retry: activeRecords=\(self.activeRecords.count), types=\(self.activityTypes.count)")
                    timer.invalidate()
                    return
                }
                self.retryCount += 1
